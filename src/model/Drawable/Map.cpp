...
void Map::Draw() {
    ...
    for (auto& hall : _halls) {
        for (auto& cell : hall->rooms_in_hall){
            ...
            monitor.draw_rectangle(cell->x, cell->y, cell->x + cell->width - 1, cell->y + cell->height - 1);
            monitor.fill_rectangle(cell->x, cell->y, cell->x + cell->width - 1, cell->y + cell->height - 1, COLOR_GREEN);
    ...
    for (auto& room : _rooms) {
        ...
        monitor.draw_rectangle(room->x, room->y, room->x + room->width - 1, room->y + room->height - 1);
        monitor.fill_rectangle(room->x, room->y, room->x + room->width - 1, room->y + room->height - 1, COLOR_RED);
    }
