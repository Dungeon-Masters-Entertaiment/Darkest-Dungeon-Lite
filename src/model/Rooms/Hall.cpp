...
void fill_rooms_in_hall(int x, int y, int x1, int y1, int type, int width, int height, std::vector<std::vector<char>> &_body, Hall &first) {
        ...
        while(i != x1) {
            if(_body[i][y] == 'C' && (last_i != i || last_i1 != y)) {
                ...
                if(first.rooms_in_hall.size() == 1) {
                    first.rooms_in_hall[first.rooms_in_hall.size() - 1] -> conection[last] = first.entrance;
                    first.entrance -> conection [(last + 2) % 4] = first.rooms_in_hall[first.rooms_in_hall.size() - 1];
                } else {
                    first.rooms_in_hall[first.rooms_in_hall.size() - 1] -> conection[last] = first.rooms_in_hall[first.rooms_in_hall.size() - 2];
                    first.rooms_in_hall[first.rooms_in_hall.size() - 2] -> conection[(last + 2) % 4] = first.rooms_in_hall[first.rooms_in_hall.size() - 1];
                }
                last_i = i;
                last_i1 = y;
            }
    ...
    first.exit -> conection[last] = first.rooms_in_hall[first.rooms_in_hall.size() - 1];
    first.rooms_in_hall[first.rooms_in_hall.size() - 1] -> conection[(last + 2) % 4] = first.exit ;
